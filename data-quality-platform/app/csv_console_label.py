...
def console_label(deduper: dedupe.api.ActiveMatching, payload) -> dict:  # pragma: no cover
    """
...
def active_train(deduper: dedupe.api.ActiveMatching, data):
    buffer_len = 1  # Max number of previous operations
    unlabeled: list[RecordDictPair] = []
    labeled: list[LabeledPair] = []
    ...
    for i in data:
        if isinstance(i, list):
            single_obj = i[0]
            if single_obj.get('user_input'):
                user_input = single_obj['user_input']
                del single_obj['user_input']
                # print(user_input,'user')
                temp_tuple = tuple(i)
                # print(temp_tuple,'tp')
                if user_input == "y":
                    ycount += 1
                    labeled.insert(0, (temp_tuple, "match"))
                elif user_input == "n":
                    ncount += 1
                    labeled.insert(0, (temp_tuple, "distinct"))
                elif user_input == "u":
                    labeled.insert(0, (temp_tuple, "unsure"))
                elif user_input == "f":
                    finished = True
                elif user_input == "p":
                    unlabeled.append(temp_tuple)
